...
var actionTypes = require('./actionTypes-7e9ccea7.js');
...
const handleConnection = (_ref2, dispatchers) => {
  ...
  disconnectSdk({
    ...
    onDisconnect: () => {
      ...
      if (userId && appId) {
        const newSdk = openChannel.Fn.init({
          appId,
          modules: [new groupChannel.$r(), new openChannel.Nn()]
        });
...
function useConnectionStatus(sdk, logger) {
  ...
  React.useEffect(() => {
    ...
    const handler = new openChannel.Pe();
